...
use Zahzah\ModuleWarehouse\Resources\StockMovement\{
    ShowStockMovement, ViewStockMovement 
};
...
class StockMovement extends MainMovement {
    ...
    protected $list       = [
        'id','parent_id','direction','card_stock_id','reference_type','reference_id',
        'item_stock_id','goods_receipt_unit_id','qty','qty_unit_id','opening_stock', 'closing_stock',
        'props'
    ];
    ...
    //SCOPE SECTION
    public function scopeIn($builder){return $builder->where('direction',self::IN);}
    public function scopeOut($builder){return $builder->where('direction',self::OUT);}
